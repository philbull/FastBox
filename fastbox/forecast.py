...
def sigmaT(expt):
    ...
    Returns:
        sigmaT (float):
            Noise rms in mK.MHz.
    """
    sigmaT2 = 4.*np.pi * expt['fsky'] * expt['Tsys']**2 / (expt['ttot']*3600. * expt['Ndish'])
...
def noise_im(cosmo, expt, ells, zmin, zmax, kmax_cutoff=False):
    ...
    if expt['type'] == 'interferometer':
        # Angular scaling
        f_ell = np.exp(_ell*(_ell+1.) * (1.22 * _lam / expt['d_max'])**2.               / (8.*np.log(2.)))
...
def number_density_to_area_density(cosmo, ngal, zmin, zmax, degrees=False):
    ...
    rmin = ccl.comoving_radial_distance(cosmo, a=1./(1.+zmin))
    rmax = ccl.comoving_radial_distance(cosmo, a=1./(1.+zmax))
    ...
    Ngal = (ngal * vol) / (4.*np.pi) # No. gals. per steradian
    if degrees:
        return Ngal * (np.pi/180.)**2. # No. gals per deg^2
    else:
...
def tracer_spectro(zmin, zmax, kind='galaxy'):
    ...
    tomo[np.where(np.logical_and(z >= zmin, z < zmax))] = 1.
    ...
    n_spectro = ccl.NumberCountsTracer(cosmo, has_rsd=False, mag_bias=None, 
                                       dndz=(z, tomo), bias=(z, bz))
    return n_spectro


def fisher_bandpowers(ells, delta_ell, fsky, Cell_gal, Cell_im, Cell_cross, Nell_gal, Nell_im):
    """
    ...
    numerator = (2.*ells + 1.) * delta_ell * fsky
    
    # Calculate denominator (product of variances)
    denom = (Cell_gal + Nell_gal) * (Cell_im + Nell_im) + Cell_cross**2. # units of (mK)^2
